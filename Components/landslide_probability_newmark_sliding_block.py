...
MISSING: Stochastic Slope, External Computation of FS Threshold
...
class LandslideProbability(Component):
    ...
    **Additional References**
    ...
    _info = {
        ...
        "topographic__slope_min": {
            "dtype": float,
            "intent": "in",
            "optional": False,
            "units": "tan theta",
            "mapping": "node",
            "doc": "min gradient of the ground surface",
        },
        "topographic__slope_mean": {
            "dtype": float,
            "intent": "in",
            "optional": False,
            "units": "tan theta",
            "mapping": "node",
            "doc": "mean gradient of the ground surface",
        },
        "topographic__slope_max": {
            "dtype": float,
            "intent": "in",
            "optional": False,
            "units": "tan theta",
            "mapping": "node",
            "doc": "max gradient of the ground surface",
        },
        "topographic__specific_contributing_area": {
            "dtype": float,
            "intent": "in",
        ...
        "peak__ground_acceleration": {
            "dtype": float,
            "intent": "in",
            "optional": False,
            "units": "(-)",
            "mapping": "node",
            "doc": "maximum peak ground acceleration across all directions",
        },
    }
    ...
    def calculate_factor_of_safety(self, i):
        ...
        self._thetamin = np.float32(self._grid.at_node["topographic__slope_min"][i])
        self._thetamean = np.float32(self._grid.at_node["topographic__slope_mean"][i])
        self._thetamax = np.float32(self._grid.at_node["topographic__slope_max"][i])
        self._theta = np.random.triangular(
            self._thetamin, self._thetamean, self._thetamax, size=self._n
        )
        ...
        self._pga = np.float32(self._grid.at_node["peak__ground_acceleration"][i])
        ...
        newmark_threshold = (self._pga / np.sin(np.arctan(self._theta))) + 1
        count = np.sum(self._FS <= newmark_threshold)
    ...
    def calculate_landslide_probability(self):
        ...
        mask = np.degrees(np.arctan(self._grid.at_node["topographic__slope_mean"][self._grid.core_nodes])) >= 15
        for i in self._grid.core_nodes[mask]:
